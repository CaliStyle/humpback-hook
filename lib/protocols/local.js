...
exports.createUser = function (_user, next) {
  //var isEmail = validateEmail( _user.identifier ); 
  var password = _user.password,
      Passport = sails.models[sails.config.permission.passportModelIdentity],
      User = sails.models[sails.config.humpback.userModelIdentity];

  /*    
  if(isEmail){
    _user.email = _user.identifier;
  }else{
    _user.username = _user.identifier;
  }
  */

  //delete _user.identifier;
  delete _user.password;
  //delete _user.password_confirm;
...
/**
 * Admin Reset local Passport for user
 *
 * This administration function can be used to reset a local Passport for a user
 * or create one if they do not have one already.
 *
 * @param {Object}   req
 * @param {Object}   res
 * @param {Function} next
 */
exports.adminreset = function (req, res, next) {
  ...
  Passport.findOne({
    protocol : 'local',
    user     : _.isObject(user) ? user.id : user
  }, function (err, passport) {
    if (err) {
      return next(err);
    }
