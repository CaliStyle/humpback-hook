        ...
        defaults: {
            ...
            models: {
                ...
                updateOrCreate: function (criteria, values, cb) {
                    ...
                    var errFind = 'No findOne() method defined!';
                    var errUpdate = 'No update() method defined!';
                    var errCreate = 'No create() method defined!';

                    // Find the connection to run this on
                    if (!_.isFunction(this.findOne)) {
                        if (deferred) {
                    ...
                    self.findOne(criteria, normalize.callback(function before(err, result) {
                        ...
                        if (result) {
                            self.update(criteria, values, normalize.callback(function afterwards(err, updatedRecords) {
                                if (err) {
                                    if (deferred) {
                                        deferred.reject(err);
                                    }
                            ...
                            self.create(values, normalize.callback(function afterwards(err, createdRecord) {
                                if (err) {
                                    if (deferred) {
                                        deferred.reject(err);
                                    }
